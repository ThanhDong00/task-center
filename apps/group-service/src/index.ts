...
async function main(): Promise<void> {
  ...
  app.post("/groups/:id/invites", async (req, res) => {
    ...
    if (await memberships.findOneBy({ groupId: found.group.id, userId: target }))
  ...
  app.get("/invitations", async (req, res) => {
    ...
    res.json(
      (
        await invitations.findBy({ userId: me.id, status: "pending" })
      ).map(publicInvitation),
    );
  });

  const answerInvite = async (req: Request<{ id: string }>, res: Response, accept: boolean) => {
    const me = requireAccess(req, res);
    if (!me) return;
  ...
  app.post("/groups/:id/leave", async (req, res) => {
    ...
    if (found.mine.role === "owner")
      return void res
        .status(400)
        .json({ error: "transfer ownership first" });
  ...
  app.post("/groups/:id/transfer", async (req, res) => {
    ...
    res.json(
      publicGroup(await groups.save(found.group), "admin"),
    );
  });
