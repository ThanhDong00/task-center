...
function validAvatar(v: unknown): v is string | null | undefined {
  return v === undefined || v === null || (typeof v === 'string' && v.length <= 2048);
}
...
async function main(): Promise<void> {
  ...
  app.post('/auth/register', async (req, res) => {
    ...
    if (!validUsername(username) || !validPassword(password))
      return void res.status(400).json({ error: 'username 1-32 chars, password 6-128 chars' });
    const name = username.trim();
    if (await repo.findOneBy({ username: name })) return void res.status(409).json({ error: 'username taken' });
  ...
  app.post('/auth/login', async (req, res) => {
    ...
    const accessToken = signJwt({ sub: found.id, username: found.username, type: 'access' }, SECRET, ACCESS_TTL_SEC);
    const refreshToken = signJwt({ sub: found.id, username: found.username, type: 'refresh' }, SECRET, REFRESH_TTL_SEC);
  ...
  app.post('/auth/refresh', async (req, res) => {
    ...
    if (!claims || claims.type !== 'refresh') return void res.status(401).json({ error: 'invalid refresh' });
  ...
  const requireAccess = async (req: Request, res: Response): Promise<UserRow | null> => {
  ...
  app.patch('/auth/me', async (req, res) => {
    ...
    if (username !== undefined) {
      if (!validUsername(username)) return void res.status(400).json({ error: 'username 1-32 chars' });
      const name = username.trim();
      if (name !== found.username && (await repo.findOneBy({ username: name })))
        return void res.status(409).json({ error: 'username taken' });
      found.username = name;
    }
    if (!validAvatar(avatar)) return void res.status(400).json({ error: 'avatar too long' });
    if (avatar !== undefined) found.avatar = avatar;
    const saved = await repo.save(found);
    const accessToken = signJwt({ sub: saved.id, username: saved.username, type: 'access' }, SECRET, ACCESS_TTL_SEC);
    res.json({ ...publicUser(saved), accessToken });
  });

  app.use((_req, res) => res.status(404).json({ error: 'not found' }));

  app.listen(PORT, () => console.log(`auth-service :${PORT} (db=${dataSource.options.database})`));
}
